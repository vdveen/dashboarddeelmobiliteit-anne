...
const DdPolicyHubsOverlay = ({
  ...
  const userData = useSelector((state: StateType) => {
    return state.authentication?.user_data || null;
  });
  ...
  const hubsPhase = getAllowedOverlayPhase(
    overlayLayers.phases[DATA_LAYER_ORDER_HUBS],
    isLoggedIn
  );
  const verbodsgebiedenPhase = getAllowedOverlayPhase(
    overlayLayers.phases[DATA_LAYER_ORDER_VERBODSGEBIEDEN],
    isLoggedIn
  );
