...
jest.mock('../../api/voiSnapshots', () => ({ fetchVoiAvailability: jest.fn() }));
...
const series = {
  ...
  series: [
    { captured_at: '2026-09-08T11:50:00Z', total: 1000, operational: 400, non_operational: 500, unknown: 100 },
    { captured_at: '2026-09-08T12:00:00Z', total: 1006, operational: 406, non_operational: 500, unknown: 100 },
  ],
};
...
test('shows a skeleton and then the latest measurement', async () => {
  ...
  expect(await screen.findByText(
    'Laatste meting: 406 van 1.006 voertuigen operationeel (40,4%)'
  )).toBeInTheDocument();
});
...
test('says so when the area never held a vehicle', async () => {
  fetchAvailability.mockResolvedValue({
    ...series,
    series: [{ captured_at: '2026-09-08T11:00:00Z', total: 0, operational: 0, non_operational: 0, unknown: 0 }],
  });

  render(<VoiAvailabilityChart polygon={polygon} onClose={jest.fn()} />);

  expect(await screen.findByText('Geen Voi-voertuigen in dit gebied in deze periode.')).toBeInTheDocument();
...
test('shows the API message and refetches on retry', async () => {
  fetchAvailability
    .mockRejectedValueOnce(new Error('De getekende vorm overlapt zichzelf. Teken het gebied opnieuw.'))
    .mockResolvedValueOnce(series);

  render(<VoiAvailabilityChart polygon={polygon} onClose={jest.fn()} />);

  await screen.findByRole('alert');
  expect(screen.getByText('De getekende vorm overlapt zichzelf. Teken het gebied opnieuw.')).toBeInTheDocument();
