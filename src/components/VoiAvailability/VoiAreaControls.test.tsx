...
const setup = (props: Partial<React.ComponentProps<typeof VoiAreaControls>> = {}) => {
  ...
  render(
    <VoiAreaControls mode={null} pointCount={0} hasPolygon={false} {...handlers} {...props} />
  );
  return handlers;
};

test('invites the user to draw and hides irrelevant actions while idle', () => {
  const handlers = setup();

  expect(
    screen.getByText('Teken een gebied om de beschikbaarheid door de tijd te zien.')
  ).toBeInTheDocument();
  expect(screen.queryByRole('button', { name: 'Wis' })).toBeNull();
...
test('explains the lasso while dragging', () => {
  ...
  expect(screen.getByRole('button', { name: 'Annuleer' })).toBeEnabled();
});
