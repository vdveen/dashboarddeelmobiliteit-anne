...
- The collector rounds `captured_at` down to the nearest ten-minute boundary, so a late container start still writes one row per boundary and the primary key keeps the run idempotent. The volume is declared at 20 GB for roughly 6 GB of positions per year.
