...
The database is managed as a Docker service because Railway's database helper does not recognize custom PostGIS images consistently. Its existing generated credentials are preserved. The PostGIS cluster uses `/var/lib/postgresql/data/postgis16` on the 20 GB persistent volume. Ten-minute collection stores roughly 6 GB per year at the current fleet size, so 20 GB leaves headroom for growth and for maintenance. `railway.ts` declares the size; growing an existing volume is a separate operation in the Railway UI. The empty PostgreSQL cluster created by Railway during initial provisioning is not used. Do not change PGDATA or detach the volume on an existing archive.
